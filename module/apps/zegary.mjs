...
export class zegarTykacza extends foundry.applications.api.ApplicationV2 {
 ...
 static DEFAULT_OPTIONS = { 
  ...
  async _renderHTML() {
    try {
      const html = await chlopcy_Utility.renderTemplate('systems/chlopcy/tameplates/app/zegar-tykacza.hbs', {
...
async close(options = {}) {
  ...
  static async initialise(tykacz) {
    ...
    if (tykacz.system.jestPrzeciwnikiem && game.user.isGM && game.combats.size === 0) {
  ...
  static async dodajPostacieDowalki(walka) {
    ...
    const template = await chlopcy_Utility.renderTemplate('systems/chlopcy/tameplates/dialog/wybierz-dziaciaki-do-walki.hbs', {
      dzieciaki,
    });
          ...
          callback: async () => {
            ...
            const combatants = wybraneDzieciaki
              .map((input, i) => {
                ...
                return actor ? { actorId: actor.id, name: actor.name, initiative: i + 1, hidden: false } : null;
              })
              .filter(Boolean);

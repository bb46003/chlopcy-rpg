...
export class dzieciak extends BaseActorSheet {
  ...
  async activateListeners(html) {
    ...
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.kostki-pyłek', (ev) => this.dawkiPylku(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', 'label.inside', (ev) => this.dawkiPylku(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.cecha-wartosc', (ev) => this.wartoscTagu(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.stan', (ev) => this.zmianaStanu(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.bangarang', (ev) => this.zmianaBangarang(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.xp', (ev) => this.zmianaXp(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.sakwy-checkbo', (ev) => this.zmianaSakwy(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '.cecha', (ev) => this.rzut(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '.dodaj-wiezi', (ev) => this.dodajWięzi(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', '.wartosc-wiezi', (ev) => this.wartoscWiezi(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '.usun-wiez', (ev) => this.usunWięzi(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '.top-section-label-bangarang', (ev) => this.uzyjPunktowBANGARANG(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'change', ".znaczniki-plecow input[type='checkbox']", (ev) => this.zmianaNaszywekPlecy(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '[data-tab="kurta-i-wspomnienia"]', (ev) => this.dynamicznyStyl(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', 'button.usun-nazywke', (ev) => this.usunNaszywki(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', 'a#label-przody', (ev) => this.dodajNaszywkę(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click contextmenu', '.naszywka-przody-obrazek', (ev) => this.owtorzNaszywke(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '[data-action="roll-dice"]', (ev) => this.rzutNaPylek(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '[for="uzycie_pylku"]', (ev) => this.uzyciePylku(ev));
    chlopcy_Utility.addHtmlEventListener(html, 'click', '.top-section-label-twardziel', (ev) => this.rzutNaTwardziela(ev));
  }
  ...
  async dodajWięzi(ev) {
    ...
    const d = new Dialog({
      ...
      buttons: '',
      render: (html) => {
        chlopcy_Utility.addHtmlEventListener(html, 'click', '.dodaj-do-katy-wiezi', (event) => this.dodajPostacDoWiezi(event));
      },
    });
    await d.render(true);
  }

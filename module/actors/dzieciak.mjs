...
export class dzieciak extends BaseActorSheet {
  ...
  async activateListeners(html) {
    ...
    html.on('click', '.top-section-label-bangarang', (ev) =>
      this.uzyjPunktowBANGARANG(ev)
    );
    html.on('change', ".znaczniki-plecow input[type='checkbox']", (ev) =>
      this.zmianaNaszywekPlecy(ev)
    );
    html.on('click', '[data-tab="kurta-i-wspomnienia"]', (ev) =>
      this.dynamicznyStyl(ev)
    );
    html.on('click', 'button.usun-nazywke', (ev) => this.usunNaszywki(ev));
    html.on('click', 'a#label-przody', (ev) => this.dodajNaszywkę(ev));
    html.on('click contextmenu', '.naszywka-przody-obrazek', (ev) =>
      this.owtorzNaszywke(ev)
    );
    html.on('click', '[data-action="roll-dice"]', (ev) => this.rzutNaPylek(ev));
    html.on('click', '[for="uzycie_pylku"]', (ev) => this.uzyciePylku(ev));
    html.on('click', '.top-section-label-twardziel', (ev) =>
      this.rzutNaTwardziela(ev)
    );
  }
  ...
  async dodajWięzi(ev) {
    ...
    const d = new Dialog({
      ...
      buttons: '',
      render: (html) => {
        html.on('click', '.dodaj-do-katy-wiezi', (event) =>
          this.dodajPostacDoWiezi(event)
        );
      },
    });
    await d.render(true);
